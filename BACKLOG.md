# BACKLOG.md — prioritert arbeidskø

Loopen konsumerer ovanfrå. `[TRENG IVER]` = berre mennesket.

## Kø

1. [ ] `[TRENG IVER]` Sjekk AHO keramikkverkstad: paste-ekstruder (WASP/Eazao?)
   + reduksjonsomn for celadon? Avgjer keramikk-tidslina.
2. [ ] `[TRENG IVER]` Vel arbeidsnamn — trengst i Salone-skjemaet.
3. [ ] `[TRENG IVER]` Start Knagg #1-printinga denne veka (kritisk sti H0).
4. [ ] **Verkstad:** Full designbrief Kråkebolle-skjerm med printbudsjett
   for wet clay (strammare overheng/spenn enn PLA).
5. [ ] **Verkstad:** Utvid skavl-algoritmen med «keramikk-modus»
   (strammare vinkel-clamp, ribbe-avstand ≥ 3× dyse).
6. [ ] **Skriving:** Redo Case #1 (Designopprøret, 4-nivå) — feila under
   Fable-grensa; køyr inline eller på nytt.
7. [ ] **Research:** Brotmodus-taksonomi reparasjonssett — feila; køyr på nytt.
8. [ ] **Research:** Celadon-kjemi + reduksjonsbrenning; norske keramikk-
   verkstader/residens (t.d. Statens verksteder, Bærum verk).
9. [ ] **Skriving:** Essay «Grown, not drawn» — formspråket som argument,
   med referanseobjekta (til Arkitektur N / Pro Forma).
10. [ ] `[TRENG IVER]` Foto 16:9 av prototypane innan ~20. aug.
11. [ ] `[TRENG IVER]` Send Salone (30/8) + Greenhouse (15/9).
12. [ ] `[TRENG IVER]` Tongji-utveksling: e-post International Office.

## Gjort

- [x] It. 0: Grunnlegging
- [x] It. 1: Konkurransefristar
- [x] It. 2–4: Briefar Knagg/Skavl-lampe/Reparasjonssett
- [x] It. 5: Norway Says
- [x] It. 6: Søknadspakke
- [x] It. 7: Manifest v1.0 → Klar
- [x] It. 8: Tongji + agent-leveransar (skavl-algoritme.md, aho-miljoet.md)
- [x] It. 9: Formspråk-pivot (07-formsprak, 3 keramikk-objekt, keramikk-print-research)
